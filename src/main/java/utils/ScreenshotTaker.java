...
public class ScreenshotTaker extends BasePage {
    ...
    public void takeFullPageScreenShot() {
        ...
        try {
            ImageIO.write(screenshot.getImage(), "jpg", new File(String.format(".//%s//%s.jpg", srcPath, srcName)));
        } catch (IOException e) {
            e.printStackTrace();
            System.out.printf("Couldn't take full page screenshot. Exception: %s", e);
        }
    }
}
